...
<c:if test="${question.typeQuestion == 'CHOIX_MULTIPLE'}">
    <div class="question-answer">
        <span class="answer-label">Choix proposés :</span>
        <ul>
            <c:forEach items="${question.choixList}" var="choix">
                <li>
                    <span class="${choix.estCorrect ? 'correct-choice' : ''}">
                        ${choix.texte}
                        <c:if test="${choix.estCorrect}"> ✅</c:if>
                    </span>
                </li>
            </c:forEach>
        </ul>
