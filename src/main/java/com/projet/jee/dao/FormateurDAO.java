...
public class FormateurDAO {
    ...
    public boolean updateFormateur(Formateur formateur) throws SQLException {
        String sqlUtilisateur = "UPDATE utilisateur SET nom = ?, prenom = ?, email = ? WHERE id = ?";
        String sqlFormateur = "UPDATE formateur SET specialite = ?, anneeExperience = ?, " +
                "certifications = ?, tarifHoraire = ?, description = ? WHERE id = ?";
        ...
        try (Connection conn = ConnectionBD.getConnection()) {
            ...
            try (PreparedStatement psUtilisateur = conn.prepareStatement(sqlUtilisateur);
                ...
                psUtilisateur.setString(1, formateur.getNom());
                psUtilisateur.setString(2, formateur.getPrenom());
                psUtilisateur.setString(3, formateur.getEmail());
                psUtilisateur.setLong(4, formateur.getId());
                int rowsUser = psUtilisateur.executeUpdate();

                // Mise à jour formateur
                psFormateur.setString(1, formateur.getSpecialite().name());
                psFormateur.setInt(2, formateur.getAnneeExperience());

                // Gérer les valeurs nulles pour certifications
                if (formateur.getCertifications() != null) {
                    psFormateur.setString(3, formateur.getCertifications());
                } else {
                    psFormateur.setNull(3, Types.VARCHAR);
                }

                psFormateur.setDouble(4, formateur.getTarifHoraire());

                // Gérer les valeurs nulles pour description
                if (formateur.getDescription() != null && !formateur.getDescription().trim().isEmpty()) {
                    psFormateur.setString(5, formateur.getDescription());
                } else {
                    psFormateur.setNull(5, Types.VARCHAR);
                }

                psFormateur.setLong(6, formateur.getId());
                int rowsFormateur = psFormateur.executeUpdate();

                conn.commit();
                return rowsUser > 0 && rowsFormateur > 0;
